...
> **Status:** the app has its dark theme (warm graphite + the user's Windows accent as the live-state "LED", all in `Theme.xaml`) and an editor pass: value-only inline editing, right-click inserts, shift-click range selection, mouse buttons as macro steps, and Run now. Latest: full mouse input as steps — all five buttons via menu or unified "Add input" capture (press any key, click, or scroll), scroll-wheel steps with editable click counts (recorded too) — infinite waits (type `inf` in a wait; parks until the macro is stopped, then the remaining steps run), and a global Enable hotkey (`RegisterHotKey`, so no hook while disarmed; works from the tray; stored in `settings.json`). Next: per-macro storage files (groundwork for import/export), then roadmap item 5 (per-app profiles) or the Home/Editor split.
...
- **Prefer the narrowest API.** For a macro that just fires on a key press, Win32 `RegisterHotKey` makes the OS notify you for that one key only — the app never sees anything else. The low-level hook is the fallback for while-held, key-up, suppression, and analog. *(Applied: the global Enable-toggle hotkey is `RegisterHotKey`-based, which is why it can work while everything is disarmed without any hook existing. Trade-off it inherits: the OS swallows that key system-wide while the app runs, so F-keys or modifier combos are the sane choices.)*
...
MacroEvent  ─┬─ KeyDown(key)        // press
             ├─ KeyUp(key)          // release
             ├─ MouseDown(button)   // left/right/middle/mouse4/mouse5
             ├─ MouseUp(button)
             ├─ Scroll(dir, clicks) // wheel up/down, N detents
             ├─ Delay(ms | ∞)       // editable wait; infinite parks until the
             │                      // macro is stopped (release / re-press),
             │                      // then the remaining steps still run
