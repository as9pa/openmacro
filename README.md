...
> **Status:** timeline editing is complete and hand-tested — every part of a macro is mutable after recording (see below). Next: editor UX pass (inline editing, drag reorder, context menus), then roadmap item 5 (per-app profiles).
...
- **`src/OpenMacro.App`** — the WPF app (code-behind for now; MVVM later, per the plan). Bindings list with enable/disable, an **Arm macros** master toggle, **Record** button, press-a-key trigger capture, and a **fully mutable timeline** — a recorded macro is a starting point, not a final product. Every step type is editable in place (delay values, text content, which key a step presses and whether it's press or release), steps insert at the selected position (key via capture, delay, text), reorder and delete work per step, **Record steps** appends a fresh recording into an existing macro, and bindings can be duplicated (copies start trigger-less so keys never conflict). Saves on every change. A **visual keyboard** (ANSI layout, collapsible) shows bound keys highlighted with the macro name on hover — click a bound key to jump to its binding, click a free key to make it the selected binding's trigger. **Minimize sends it to the tray** (arm/disarm and exit from the tray menu); closing the window exits fully and uninstalls everything, per the "nothing lingers" decision. `HookService` makes the safety rule real: the global hook exists **only** while armed, recording, or capturing a trigger — an idle window has no hook installed. Single-instance guard (a second copy just tells you it's already running).
